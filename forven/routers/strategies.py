...
def get_ai_dropzone_context():
    ...
    return {
        ...
        "gotchas": [
            ...
            "Trading short or both sides? Declare BOTH: `supported_trade_modes = {'long_only', 'short_only', 'both'}` (class attribute, the subset you actually implement) AND `trade_mode` in default_params ('both'/'short_only'). Without either declaration the engine defaults to long_only and rejects the run with \"does not support trade_mode='both'\" — an explicit trade_mode in YOUR default_params is honored, but a mode passed only as a backtest request override on an undeclared class is not.",
            ...
            "Signal contract: `from forven.strategies.base import BaseStrategy, Signal` — this is the ONLY allowlisted import path (there is NO forven.strategies.signal module; guessing one costs a rejected registration). Signal is a DATACLASS, not an enum: fields are entry_signal/exit_signal (bool), price (float), direction ('long'|'short'), confidence (float), indicators (dict). There is no Signal.NEUTRAL/BUY/SELL — 'no trade' is Signal(entry_signal=False). trade_mode values are the strings 'long_only'|'short_only'|'both'.",
            ...
            "funding_rate is a PER-HOUR rate (Binance 8h rates are divided by 8; HL is natively hourly): typical |values| are 1e-6..1e-5 and the long-run median is ~+5e-6. Absolute thresholds calibrated to per-8h intuition (e.g. 'extreme = 0.01% = 1e-4', or bps-scale filters) are ~8x too strict against this column and produce permanent zero-trade strategies — divide per-8h thresholds by 8, or use z-scores/rolling percentiles which are scale-free.",
